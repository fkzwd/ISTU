/*
àïèñàòü ïðîãðàììó, âûïîëí¤þùóþ ðàñ÷åò ïðîèçâåäåíè¤ äâóõ íåêâàäðàòíûõ ìàòðèö.
žàòðèöû äîëæíû õðàíèòüñ¤ â ïàì¤òè â âèäå äâóìåðíîãî äèíàìè÷åñêîãî ìàññèâà,
ðàçìåðíîñòè ìàòðèö ââîä¤òñ¤ ïîëüçîâàòåëåì ñ êëàâèàòóðû.
¾ðåäóñìîòðåòü ãåíåðàöèþ çíà÷åíèé ìàòðèö êàê ñëó÷àéíûõ âåùåñòâåííûõ ÷èñåë â äèàïàçîíå îò -0.5 äî 0.5.
*/
...
double random(double max, double min) {
	...
	return (int) result; //тут надо написать "return result;" чтобы возвращало от -0.5 до 0.5 и заменить параметры вызова в 76 строке
}
...
void generateArray(double **array, int y, int x) {
	for (int i = 0; i<y; i++){
		for (int j = 0; j<x; j++) {
			array[i][j] = random(3, 1); // заменить на "array[i][j] = random(0.5, -0.5);"
		}
	}
}
